...
class EditBiteBagLogic extends GetxController {
  final state = EditBiteBagState();
  ...
  Future<firebase_storage.UploadTask?> uploadFile(
    ...
    if (file == null) {
      try {
        FirebaseFirestore.instance.collection('biteBags').doc(id).update({
          'chef_note': chefNoteController.text,
          'category': selectedCategories,
          'quantity': int.parse(quantityController.text.toString()),
          'original_price':
              double.parse(originalPriceController.text.toString())
                  .toPrecision(2),
      ...
      log('URL---->>$downloadURL');
      try {
        FirebaseFirestore.instance.collection('biteBags').doc(id).set({
          'chef_note': chefNoteController.text,
          'category': selectedCategories,
          'quantity': int.parse(quantityController.text.toString()),
